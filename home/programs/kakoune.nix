{pkgs, ...}: {
  ...
  extraConfig = ''
    ...
    set-face global PrimaryCursor      "%opt{surface2},%opt{lavender}"
    set-face global SecondaryCursor    "%opt{text},%opt{overlay0}"
    set-face global PrimaryCursorEol   "%opt{surface2},%opt{lavender}"
    set-face global SecondaryCursorEol "%opt{text},%opt{overlay0}"

    hook global ModeChange .*:insert %{
        set-face window PrimaryCursor "%opt{crust},%opt{rosewater}"
        set-face window PrimaryCursorEol "%opt{crust},%opt{rosewater}"
    }
